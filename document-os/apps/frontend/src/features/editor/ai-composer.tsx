...
const AGENTS = [
  { id: "auto", label: "Auto (Planner + Writer)", desc: "Outlines & generates document" },
  { id: "planner", label: "Planner Only", desc: "Drafts section structure" },
  { id: "writer", label: "Writer Only", desc: "Writes & refines content" },
  { id: "reviewer", label: "Reviewer Only", desc: "Scores quality & reviews" },
] as const;
  ...
  if (minimized) {
    return (
      <div className="fixed inset-x-0 bottom-6 z-[100] flex justify-center px-4 pointer-events-none">
        ...
        >
          <img src="/logo.png" alt="Logo" className="h-4 w-4 object-contain shrink-0" />
          ...
          <div className="flex items-center justify-between pb-2 select-none">
            ...
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 px-2.5 py-0.5 text-[11px] font-semibold text-[#5551FF] dark:bg-indigo-500/20 dark:border-indigo-500/30 dark:text-indigo-300 shadow-xs">
                <img src="/logo.png" alt="Logo" className="h-3.5 w-3.5 object-contain shrink-0" />
                Gemma 4
              </span>
              <button
                type="button"
            ...
            <div className="flex items-center gap-2 min-w-0 flex-1 overflow-x-auto py-0.5">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    type="button"
                    className="flex items-center gap-1.5 rounded-full border border-indigo-200/80 dark:border-indigo-800/80 bg-indigo-50/80 dark:bg-indigo-950/40 px-3 py-1 text-xs font-semibold text-indigo-950 dark:text-indigo-200 shadow-xs hover:border-[#5551FF] hover:bg-indigo-100/90 dark:hover:bg-indigo-900/60 transition-all cursor-pointer"
                  >
                    <img src="/logo.png" alt="Logo" className="h-3.5 w-3.5 object-contain shrink-0" />
                    <span>{AGENTS.find((a) => a.id === agent)?.label ?? "Auto"}</span>
                    <ChevronDown className="h-3.5 w-3.5 opacity-60 text-indigo-600 dark:text-indigo-400" />
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent
                  align="start"
                  side="top"
                  sideOffset={8}
                  className="z-[110] min-w-[230px] rounded-2xl border border-indigo-200/80 dark:border-indigo-900/70 bg-popover/95 p-1.5 shadow-2xl backdrop-blur-md"
                >
                  <div className="px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/80">
                    Agent Strategy
                  </div>
                  {AGENTS.map((a) => {
                    ...
                    return (
                      <DropdownMenuItem
                        ...
                        className={cn(
                          "flex cursor-pointer items-center justify-between rounded-xl px-2.5 py-2 text-xs transition-all my-0.5",
                          isSelected
                            ? "bg-[#5551FF]/15 text-[#5551FF] font-semibold dark:bg-[#5551FF]/25 dark:text-indigo-300"
                            : "hover:bg-accent hover:text-foreground text-foreground/85",
                        )}
                      >
                        <div className="flex flex-col gap-0.5 min-w-0 pr-2">
                          <span className="font-semibold text-xs leading-tight">{a.label}</span>
                          <span className="text-[10.5px] text-muted-foreground/70">{a.desc}</span>
                        </div>
                        {isSelected && <Check className="h-3.5 w-3.5 shrink-0 text-[#5551FF] dark:text-indigo-300" />}
                      </DropdownMenuItem>
                    );
                  })}
                </DropdownMenuContent>
